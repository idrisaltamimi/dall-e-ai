...
const CreatePost = () => {
  ...
  const generateImage = async () => {
    if (form.prompt) {
      try {
        setGenerateImg(true)
        const response = await fetch('https://dalle-j97c.onrender.com/api/v1/dalle',
          {
            method: 'POST',
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (form.prompt && form.photo) {
      ...
      try {
        const response = await fetch('https://dalle-j97c.onrender.com/api/v1/post',
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
      ...
      <form className='mt-16 max-w-3xl' onSubmit={handleSubmit}>
        ...
        <div className='mt-10'>
          <p className='mt-2 text-[#666e75] text-[14px]'>
            One you have created the image you created, you can share it with others in the community
          </p>
          <button
            type='submit'
            className='mt-3 text-white bg-[#6496ff] font-medium rounded-md text-sm w-full sm:w-auto px-5 py-2.5 text-center'
          >
